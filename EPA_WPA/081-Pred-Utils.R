...
epa_fg_probs <- function(dat, current_probs,ep_model, fg_mod) {
  fg_test_attempt_i = str_detect((dat$play_type), "Field Goal")
  fg_test_data <- dat[fg_test_attempt_i, ]
  ...
  end_game_i <-
    which(fg_test_data$TimeSecsRem <= 0)
  ...
  current_probs[fg_test_attempt_i, "FG"] <- make_fg_prob[fg_test_attempt_i] + 
    missed_fg_ep_preds[fg_test_attempt_i,"Opp_FG"]
  # Update the other columns based on the opposite possession:
  current_probs[fg_test_attempt_i, "TD"] <- missed_fg_ep_preds[fg_test_attempt_i, "Opp_TD"]
  current_probs[fg_test_attempt_i, "Opp_FG"] <- missed_fg_ep_preds[fg_test_attempt_i, "FG"]
  current_probs[fg_test_attempt_i, "Opp_TD"] <- missed_fg_ep_preds[fg_test_attempt_i, "TD"]
  current_probs[fg_test_attempt_i, "Safety"] <- missed_fg_ep_preds[fg_test_attempt_i, "Opp_Safety"]
  current_probs[fg_test_attempt_i, "Opp_Safety"] <- missed_fg_ep_preds[fg_test_attempt_i, "Safety"]
  current_probs[fg_test_attempt_i, "No_Score"] <- missed_fg_ep_preds[fg_test_attempt_i, "No_Score"]
...
calculate_epa_local <- function(clean_pbp_dat, ep_model, fg_model) {
  ...
  # define turnover flag
  turnover_ind = clean_pbp_dat$play_type %in% turnover_play_type
  clean_pbp_dat$turnover = 0
  
  # define turnover on downs
  downs_turnover = (clean_pbp_dat$play_type %in% normalplay & clean_pbp_dat$down == 4)
  
  # data is ordered
  new_offense = !(clean_pbp_dat$offense_play == lead(clean_pbp_dat$offense_play, order_by = clean_pbp_dat$id_play))
  scoring_plays = clean_pbp_dat$play_type %in% offense_score_vec
  
  # end of half check as well
  end_of_half_plays = !(clean_pbp_dat$half == lead(clean_pbp_dat$half,order_by = clean_pbp_dat$id_play))
  
  # is specifically defined as a turnover
  turnover_play_check = clean_pbp_dat$play_type %in% turnover_play_type 
  
  # turnovers only occur on actual change of offense
  # but not scoring plays
  # and not at the end of half.
  # Turnovers now capture downs turnovers, when there is a change of offense after a fourth down normal play.
  t_ind = (turnover_ind | (new_offense)) & !scoring_plays & !end_of_half_plays & (turnover_play_check | downs_turnover)
  
  clean_pbp_dat$turnover[t_ind] <- 1
  ...
  pred_df = pred_df %>%
    ...
    select(-log_ydstogo,-log_ydstogo_end,
...
calculate_epa_local_alt <- function(clean_pbp_dat, ep_model, fg_model) {
  ##--Play type vectors------
  turnover_play_type = c(
    "Blocked Field Goal",
    "Blocked Field Goal Touchdown",
  ...
  # ep_start
  ep_start = as.data.frame(predict(ep_model, pred_df_before, type = 'prob'))
  ...
  pred_df_before$ep_before = apply(ep_start_update, 1, function(row) {
    sum(row * weights)
  })
  colnames(ep_start_update) <- paste0(colnames(ep_start_update), "_pre")
  pred_df_before <- cbind(pred_df_before,ep_start_update)
  
  pred_df_after = clean_pbp_dat %>% 
    arrange(id_play) %>%  
    select(
      id_play,
      drive_id,
  ...
  colnames(pred_df_after) = gsub("new_", "", colnames(pred_df_after))
  ...
  colnames(pred_df_after)[4:11] = paste0(colnames(pred_df_after)[4:11], "_end")
  colnames(ep_end) <- paste0(colnames(ep_end), "_post")
  ...
  # print("Number of rows in pbp dataframe")
  # print(nrow(clean_pbp_dat))
  # summary(pred_df_before)
  # summary(pred_df_after)
  pred_df <- clean_pbp_dat %>%
    left_join(pred_df_before, by = c("id_play", 
                                     "drive_id", 
                                     "game_id",
                                     "TimeSecsRem",
                                     "down",
                                     ...
                                     "Goal_To_Go",
                                     "home_offense")) %>% 
    left_join(pred_df_after, by = c("id_play", "drive_id", "game_id"))
